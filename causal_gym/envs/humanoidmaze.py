...
class HumanoidMazeSCM(SCM):
    """
    Confounded HumanoidMaze environment with latent wind field.
    ...
    Observation space (69D base + 2D W):
        ...
        V  (36-38): 3D  - Torso vertical (HIDDEN from imitator)
        C  (39-41): 3D  - COM (center of mass) velocity
        J  (42-68): 27D - Joint velocities (root 6D + hinges 21D)
        W  (new):   2D  - Noisy perceived tilt (observed by imitator)
        X  (action):21D - Joint torques

    Confounding:
        - V (torso_vertical): Expert-only, critical for balance
        - W (perceived tilt): Noisy proxy mixing V's XY tilt with wind direction
        - U (wind field): Latent, affects dynamics, W, and reward Y
    """
    ...
    def _U_val(self) -> NDArray[np.float64]:
        """Compute wind field value (piecewise-constant gust process)."""
        p_gust = 0.05
        min_strength = 2.0
        max_strength = 5.0
    ...
    def _W(self) -> NDArray[np.float64]:
        """
        Compute noisy perceived tilt.

        V = torso_vertical is a 3D up-vector.
        When upright: V ~ [0, 0, 1]
        Tilt in XY plane: V[:2] tells how much we're leaning.

        W mixes this tilt signal with wind direction, creating a biased proxy.
        """
        u = self._U[-1]
        ...
        # Get XY tilt from torso vertical
        v = self.V[-1]
        tilt_xy = v[:2]  # How much torso leans in X and Y

        alpha_v = 0.9   # Dominant tilt signal
        alpha_u = 0.1   # Small wind contamination
        noise_std = 0.05

        base = alpha_v * tilt_xy + alpha_u * u_hat
    ...
    def _reward(self) -> float:
        """Compute reward with wind penalty."""
        ag = np.asarray(self.P[-1][:2], dtype=np.float64)
        dg = np.asarray(self._goal_xy, dtype=np.float64)
        dist = np.linalg.norm(ag - dg)
    ...
    def get_graph(self) -> Graph:
        ...
        for step in range(H):
            ...
            # Wind confounding: V -> W
            base_graph[v][w] = 1

            # Bidirected edges (confounding via U)
            conf_graph[w][y] = 1
            conf_graph[c][y] = 1
            conf_graph[w][c] = 1
